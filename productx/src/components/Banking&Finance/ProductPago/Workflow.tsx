  ...
  const Workflow = () => {
    ...
    // Scroll detection with slower step change
    useEffect(() => {
      const scrollableElement = scrollableContainerRef?.current;
      let ticking = false;

      const handleScroll = () => {
        if (!scrollableElement || ticking) return;
        ...
        requestAnimationFrame(() => {
          ...
          // Update only if significantly close to center (slower change)
          if (minDistance < viewportHeight * 2 && closestStep !== activeStep) {
    ...
    }, [scrollableContainerRef, activeStep]);

    return (
      <div className="w-full min-h-screen bg-[#a50044] py-20 px-10">
        {/* Title */}
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          {/* Right - Steps */}
          <div className="relative flex flex-col gap-6 px-5 md:px-10">
            {steps.map((text, idx) => (
              <div 
                key={idx} 
              ...
              >
                <motion.div
                  ...
                  animate={{
                    ...
                    backgroundColor: activeStep === idx ? "#fff" : "transparent",
