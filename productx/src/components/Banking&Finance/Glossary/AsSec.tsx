...
useEffect(() => {
  const handleScroll = () => {
    ...
    if (scrollY <= 0) {
      // top of page
      setStickyOffset(isMobile ? 80 : 112);
    } else {
      // after scroll
      setStickyOffset(isMobile ? 0 : 72);
    }
  };
...
<div
className="sticky z-[10000] bg-black text-white border-b border-gray-700"
...
className={`snap-center text-xl lg:text-2xl font-semibold transition-colors ${
activeLetter === letter
? "text-[#FFB800]"
...
openItem === entry.term ? "max-h-[500px] mt-2" : "max-h-0"
}`}
>
