...
export default function FlyingPosters({
    ...
    const [waveOffset, setWaveOffset] = useState(0); // 0 → 100 as we scroll
    ...
    // Dynamic opacity using Intersection Observer - increases as you scroll DOWN
    useEffect(() => {
        ...
        const observerCallback = (entries: IntersectionObserverEntry[]) => {
            entries.forEach((entry) => {
                ...
                // How much the top of the section has moved up (0 → positive as you scroll down)
                const distanceScrolled = Math.abs(Math.min(0, rect.top));

                // Limit the animation distance (1x viewport height – tweak as you like)
                const maxDistance = windowHeight * 1.0;

                // Progress: 0 → 1 within that distance
                const rawProgress = distanceScrolled / maxDistance;
                const scrollProgress = Math.min(1, Math.max(0, rawProgress));

                // If you are happy with current direction, keep this
                const invertedProgress = 1 - scrollProgress;

                // Opacity (already working for you)
                const newOpacity = 0.2 + invertedProgress * 0.8;
                setWaveOpacity(newOpacity);

                // ✅ NEW: background movement (0 → 30% shift for example)
                const maxShift = 50; // change this value to move more/less
                const newOffset = invertedProgress * maxShift;
    ...
    return (
        <div className="relative w-full bg-black min-h-[320vh] md:min-h-[420vh] lg:min-h-[500vh]">
            {/* Canvas Container - Flying Images Animation */}
            <div
                ref={containerRef}
                className={`w-full h-screen overflow-hidden sticky top-0 z-[1] ${className ?? ''}`}
                {...props}
            >
                <canvas ref={canvasRef} className="block w-full h-full absolute top-0 left-0 sm:left-0 lg:left-100" />
            </div>

            {/* Background Wave Image - Starts Left Top, Ends Right Bottom */}
            <div
                className="absolute w-full z-0 transition-[opacity,background-position] duration-300 ease-out"
                style={{
                    top: '0',
                    left: '0',
                    height: '90%',
                    ...
                    backgroundSize: '120% 100%', // slight zoom for nicer parallax
                    // X: 10% → 110% as scrollProgress goes 0 → 1
                    backgroundPosition: `${10 + waveOffset}% 20%`,
                    backgroundRepeat: 'no-repeat',
                    opacity: waveOpacity,
                }}
            />

            {/* Content Container - ABSOLUTE positioning for smooth scroll */}
            <div
      ...
      "
            ...
            "
                        ...
                        <h3
                            className="
              text-[20px] sm:text-[28px] lg:text-[40px]
            "
                            style={{
                ...
                <ScrollTextSection sections={textSections} />

                {/* Bottom Section - Description and Button */}
                <div className="hidden lg:flex min-h-screen items-center">
                    <div className="pt-8 sm:pt-10 lg:pt-12 max-w-2xl">
                        <p
                            className="
              mb-8
              text-[14px] sm:text-[16px] lg:text-[26px]
            "
                            style={{
                                fontFamily: 'Bricolage Grotesque, sans-serif',
                                fontWeight: 700,
