...
interface IndustryCardProps {
  ...
  isActive: boolean;
  isReady: boolean;
  isMobile: boolean; // New prop to determine mobile behavior
  onClick: () => void;
  url: string;
...
const IndustryCard = React.forwardRef<HTMLDivElement, IndustryCardProps>(
  ({ title, image, isReady, isMobile, onClick, url, onComingSoonClick }, ref) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Overlay */}
          <div className={`absolute inset-0 flex flex-col justify-between p-4 transition-all duration-500 z-10 ${
            isReady 
              ? 'bg-transparent' 
              : showComingSoonText 
                ? 'bg-black/75' 
            ...
            <div className="relative z-20 w-full mt-auto">
              <div className="flex justify-between items-center w-full">
                <span className={`font-semibold text-base lg:text-base tracking-wider transition-all ${
                  isReady 
