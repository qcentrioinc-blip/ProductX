  ...
  ({ title, image, isActive, onClick, url }, ref) => (
    ...
    >
      ...
      <a
        ...
        className={`block relative overflow-hidden rounded-xl transition-all duration-700 ${isActive ? 'shadow-[0_0_40px_rgba(255,255,255,0.2)]' : ''
          }`}
      ...
      >
        ...
        <div className={`hidden xl:flex absolute inset-0 bg-black/40 transition-opacity duration-700 items-end p-4 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
          }`}>
          <div className={`flex justify-between items-center w-full transition-transform duration-700 ${isActive ? 'translate-y-0' : 'translate-y-4 group-hover:translate-y-0'
            }`}>
      ...
      <div className="xl:hidden mt-4 w-full max-w-[260px]">
        <div className="flex justify-between items-center w-full">
          <span className={`text-white font-semibold text-base tracking-tight transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-60'
            }`}>
            {title}
          </span>
          <div className={`p-1.5 rounded-full backdrop-blur-md transition-all duration-300 ${isActive ? 'bg-white/20' : 'bg-white/10'
            }`}>
            <ArrowUpRight size={16} className="text-white" />
          </div>
        </div>
