...
const VisionImpact: React.FC = () => {
    ...
    // Apply spring to all transforms for smooth motion
    const leftImageX = useSpring(useTransform(scrollYProgress, [0.2, 0.6], [0, -700]), springConfig);
    const leftImageY = useSpring(useTransform(scrollYProgress, [0.2, 0.6], [0, -300]), springConfig);
    const leftImageOpacity = useSpring(useTransform(scrollYProgress, [0.2, 0.5, 0.9], [1, 1, 0]), springConfig);

    const rightImageX = useSpring(useTransform(scrollYProgress, [0.2, 0.6], [0, 700]), springConfig);
    const rightImageY = useSpring(useTransform(scrollYProgress, [0.2, 0.6], [0, -300]), springConfig);
    const rightImageOpacity = useSpring(useTransform(scrollYProgress, [0.2, 0.5, 0.9], [1, 1, 0]), springConfig);

    // Center image transforms with spring
    const centerImageScaleX = useSpring(useTransform(scrollYProgress, [0.2, 1.2], [1, 15]), springConfig);
    const centerImageScaleY = useSpring(useTransform(scrollYProgress, [0.2, 2.5], [1, 10]), springConfig);
    const centerImageZ = useSpring(useTransform(scrollYProgress, [0.2, 1.8], [10, 200]), springConfig);
    const centerImageOpacity = useSpring(useTransform(scrollYProgress, [0.2, 1], [1, 0.8]), springConfig);

    // Text animations with spring
    const textOpacity = useSpring(useTransform(scrollYProgress, [0.5, 0.8], [0, 1]), springConfig);
    const textY = useSpring(useTransform(scrollYProgress, [0.2, 0.4], [100, 0]), springConfig);

    return (
        <div className="bg-black min-h-screen py-2 sm:py-4 md:py-6 lg:py-8 xl:py-12 2xl:py-16" id="visionimpact">
            {/* Title Section - Smooth spring entrance */}
            <motion.div
            ...
            >
                <div className="text-center -mb-8 sm:mb-0">
                    ...
                    <motion.h1
                        className="text-xl sm:text-2xl md:text-3xl lg:text-6xl xl:text-7xl mb-0 sm:mb-2 md:mb-3 lg:my-6 xl:my-8 font-bold text-white tracking-tight"
                        initial={{ opacity: 0, y: 30 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ 
                            type: "spring",
            ...
            <div className="bg-black">
                ...
                >
                    <div className="sticky top-0 h-screen flex items-center justify-center overflow-hidden">
                        <motion.div
                            ...
                            style={{ perspective: '2000px' }}
                        >
                            {/* Left Images - V-shape movement with spring */}
                            <motion.div
                                className="flex gap-0.5 sm:gap-1 md:gap-2 lg:gap-3 xl:gap-4"
                                style={{
                                    x: leftImageX,
                            ...
                            {/* Center Image - Smooth spring scaling */}
                            <motion.div
                                className="relative mx-0.5 sm:mx-1 md:mx-1.5 lg:mx-2 xl:mx-3 2xl:mx-4"
                                style={{
                                    scaleX: centerImageScaleX,
                            ...
                            {/* Right Images - V-shape movement with spring */}
                            <motion.div
                                className="flex gap-0.5 sm:gap-1 md:gap-2 lg:gap-3 xl:gap-4"
                                style={{
                                    x: rightImageX,
                        ...
                        {/* Text overlay with smooth spring animation */}
                        <motion.div
                            className="absolute inset-0 bottom-20 flex items-center justify-center md:justify-end z-50 pointer-events-none"
                            style={{
                                opacity: textOpacity,
                ...
                {/* Final Text Block with smooth entrance */}
                <div className="w-full px-3 sm:px-4 md:px-6 lg:px-7 xl:px-8 -mt-[30vh] sm:-mt-[40vh] md:-mt-[16vh] lg:mt-0">
                    <motion.div
                        className="flex justify-center md:justify-end"
                        initial={{ opacity: 0, y: 50 }}
                        whileInView={{ opacity: 1, y: 0 }}
